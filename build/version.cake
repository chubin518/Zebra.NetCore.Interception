...
public class BuildParameters
{
	...
	private void InitializeVersion()
	{
		...
		var versionSuffix = doc.DocumentElement.SelectSingleNode("/Project/PropertyGroup/VersionSuffix").InnerText;
	
		Version =	new BuildVersion(int.Parse(versionMajor), int.Parse(versionMinor), int.Parse(versionPatch), versionQuality);
		Version.Suffix =string.IsNullOrWhiteSpace(versionSuffix)?null: versionSuffix;
	}
}
